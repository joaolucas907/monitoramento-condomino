...
if($usuario!=""){
    $sql_visita = $pdo->query("SELECT * from visitante where usuario = '$usuario'");  
...
/*
if($morador !=""){
    $sql_morador = $pdo->query("SELECT * from cadastro where usuario = '$morador'");
    ...
    echo "coloque o nome do responsavel";
    exit();
}
*/
if($imagem["name"] != ""){
...
$res = $pdo->prepare("INSERT into visitante (usuario, morador, imagem, monitoramento, situacao) values (?,?,?,'fora','ativado')");

$res->bindParam(1, $usuario,PDO::PARAM_STR);
$res->bindParam(2, $_SESSION['usuario'],PDO::PARAM_STR);
